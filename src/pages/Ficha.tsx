...
export function Ficha() {
  ...
  return (
    <div className="min-h-screen bg-[#131110]">
      {/* Topbar */}
      <header className="no-print sticky top-0 z-40 flex items-center justify-between h-[60px] px-7 bg-[#161311] border-b border-white/[0.06]">
        ...
        <div className="flex items-center gap-[14px]">
          ...
          <div className="flex items-center gap-1">
            <input
              ...
              className="w-16 bg-white/5 border border-white/10 rounded-[7px] px-2 py-1.5 text-[#F5F0E8] text-xs placeholder:text-[#6B6560] focus:outline-none focus:border-[rgba(212,160,23,0.5)]"
            />
            <button
              onClick={handleGanharXP}
              disabled={!xpInput || parseInt(xpInput) <= 0}
              className="text-[13px] font-bold text-[#131110] bg-[#D4A017] hover:bg-[#E8C25A] border-0 rounded-[9px] px-[14px] py-[9px] cursor-pointer transition-colors disabled:opacity-40 disabled:cursor-default"
            >
              + XP
            </button>
          </div>
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 text-[13px] font-semibold text-[#A8A09B] hover:text-[#E8DFD0] bg-white/[0.04] border border-white/[0.08] rounded-[9px] px-3 py-2 cursor-pointer transition-colors"
          >
          ...
          <span className="inline-flex items-center gap-[7px] text-[#7c9b6e] text-[13px] font-medium">
      ...
      {/* Main content */}
      <div className="max-w-[1180px] mx-auto px-8 py-8 pb-14">

        {/* Character header */}
        <div className="flex items-start justify-between gap-7 mb-7 flex-wrap">
          {/* Name + badges */}
          <div className="flex items-center gap-[18px]">
            <div className="w-16 h-16 rounded-2xl flex-shrink-0 bg-[#221d18] border border-[rgba(212,160,23,0.35)] overflow-hidden">
              <CharacterAvatar nome={identity.nome_personagem} id={fichaId ?? id} size={64} />
            </div>
            <div>
              <h2 className="font-extrabold text-[26px] leading-tight tracking-tight text-[#F5F0E8] mb-2">
          ...
          {/* XP bar */}
          <div className="min-w-[300px] flex-1 max-w-[360px]">
        ...
        <div className="grid gap-4 mb-[14px]" style={{ gridTemplateColumns: '1.5fr 1fr 1fr 1fr' }}>
          {/* HP card */}
          <div
            className="rounded-2xl p-[18px_20px]"
            style={{
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <span className="inline-flex items-center gap-2 text-xs font-semibold tracking-[0.08em] uppercase text-[#e0a3a3]">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="#d4564a"><path d="M12 21s-7-4.35-9.5-8.5C.5 9 2 5 5.5 5 7.5 5 9 6.2 12 9c3-2.8 4.5-4 6.5-4C22 5 23.5 9 21.5 12.5 19 16.65 12 21 12 21z"/></svg>
                Vida
              </span>
              {/* Quick HP adjust */}
              <div className="flex items-center gap-1">
                <input
                  ...
                  className="w-12 text-center bg-black/20 border border-[rgba(181,57,47,0.4)] rounded-[6px] px-1 py-1 text-[#F5F0E8] text-xs focus:outline-none"
                />
                <button
                  onClick={() => { const n = parseInt(pvDelta); if (!isNaN(n)) handleAjustarPV(-Math.abs(n)) }}
                  aria-label="Aplicar dano"
                  className="w-[26px] h-[26px] rounded-[7px] bg-[rgba(181,57,47,0.18)] border border-[rgba(181,57,47,0.5)] text-[#e0a3a3] text-[15px] font-bold cursor-pointer flex items-center justify-center hover:bg-[rgba(181,57,47,0.3)] transition-colors"
                >−</button>
                <button
                  onClick={() => { const n = parseInt(pvDelta); if (!isNaN(n)) handleAjustarPV(Math.abs(n)) }}
                  aria-label="Aplicar cura"
                  className="w-[26px] h-[26px] rounded-[7px] bg-[rgba(124,155,110,0.18)] border border-[rgba(124,155,110,0.5)] text-[#9cc090] text-[15px] font-bold cursor-pointer flex items-center justify-center hover:bg-[rgba(124,155,110,0.3)] transition-colors"
                >+</button>
              </div>
            </div>
            <div className="flex items-baseline gap-[7px] whitespace-nowrap">
              <span className="font-extrabold text-[42px] leading-none text-[#F5F0E8] tracking-tight">{pvAtual}</span>
              <span className="text-[19px] font-semibold text-[#c98c8c]">/ {pvMax}</span>
            </div>
          ...
          {/* CA */}
          <div className="vg-card p-[18px_20px] flex flex-col justify-between">
            <span className="inline-flex items-center gap-2 text-xs font-semibold tracking-[0.08em] uppercase text-[#8a8278]">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#B8860B" strokeWidth="1.5"><path d="M12 2l7 3v6c0 4.5-3 8.5-7 9.5C8 19.5 5 15.5 5 11V5z"/></svg>
              Armadura
            </span>
            <div className="font-extrabold text-[38px] leading-none tracking-tight text-[#F5F0E8] mt-4">
          ...
          <div className="vg-card p-[18px_20px] flex flex-col justify-between">
            <span className="inline-flex items-center gap-2 text-xs font-semibold tracking-[0.08em] uppercase text-[#8a8278]">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#B8860B" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M13 2L3 14h7l-1 8 10-12h-7z"/></svg>
              Iniciativa
            </span>
            <div className="font-extrabold text-[38px] leading-none tracking-tight text-[#F5F0E8] mt-4">
          ...
          <div className="vg-card p-[18px_20px] flex flex-col justify-between">
            <span className="inline-flex items-center gap-2 text-xs font-semibold tracking-[0.08em] uppercase text-[#8a8278]">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#B8860B" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M13 4v7l5 3M4 7l3 2-2 4 4 1 1 5"/></svg>
              Deslocamento
            </span>
            <div className="font-extrabold text-[38px] leading-none tracking-tight text-[#F5F0E8] mt-4">
              {combate.deslocamento._total_metros !== null ? (
                <>{combate.deslocamento._total_metros}<span className="text-[17px] text-[#6B6560] font-semibold"> m</span></>
              ) : '—'}
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-[10px] mb-[30px]">
          {[
            ...
            <div key={label} className="flex-1 flex items-center justify-center gap-[9px] py-3 rounded-[12px] bg-[#1A1714] border border-[rgba(212,160,23,0.18)]">
              <span className="text-[13px] text-[#8a8278] font-medium">{label}</span>
